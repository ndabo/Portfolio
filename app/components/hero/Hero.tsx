...
import { FaArrowRight, FaFileAlt, FaEnvelope } from 'react-icons/fa'
...
const TAGS = ['AI / ML', 'Data Science', 'Software Engineering', 'Systems & Networks']
...
export default function Hero() {
  ...
  // Parallax: photo drifts up, ambient glow drifts down as you scroll past.
  ...
  return (
    ...
    >
      ...
      {/* Radial crimson ambient glow — top left */}
      <motion.div
        style={{ y: glowY }}
        className="absolute -top-32 -left-32 w-[600px] h-[600px] bg-primary/10 rounded-full blur-[120px] pointer-events-none"
      />
      {/* Electric cyan glow — bottom right (data accent) */}
      <motion.div
        style={{ y: glowY }}
        className="absolute -bottom-20 right-0 w-[420px] h-[420px] bg-accent-blue/[0.07] rounded-full blur-[110px] pointer-events-none"
      />
      ...
      <div className="relative z-10 flex flex-1 flex-col gap-8 px-6 md:px-12 lg:px-24 pt-14 pb-6 md:flex-row md:items-center max-w-[1280px] mx-auto w-full">
        ...
        >
          <motion.div style={{ y: photoY }} className="relative h-full w-full">
            {/* Outer glow */}
            <div className="absolute -inset-6 bg-primary/10 rounded-2xl blur-3xl" />

            {/* Corner accent marks — diagonal crimson / cyan */}
            <div className="absolute top-0 left-0 w-6 h-6 border-t-2 border-l-2 border-primary z-20 rounded-tl" />
            <div className="absolute top-0 right-0 w-6 h-6 border-t-2 border-r-2 border-accent-blue z-20 rounded-tr" />
            <div className="absolute bottom-0 left-0 w-6 h-6 border-b-2 border-l-2 border-accent-blue z-20 rounded-bl" />
            ...
            <div className="relative h-full w-full rounded-xl overflow-hidden bg-bg-card border border-primary/20">
              ...
              <div className="absolute bottom-4 left-4 right-4 z-10">
                <div className="flex items-center justify-between bg-bg-dark/85 backdrop-blur-md px-4 py-3 rounded-lg border border-border-dark">
                  ...
                  <span className="flex items-center gap-1.5 text-xs text-accent-blue font-mono font-bold">
        ...
        {/* ── Right: Content ── */}
        <motion.div style={{ y: contentY }} className="flex flex-col gap-5 flex-1 md:pl-8 lg:pl-12">
          ...
          <motion.div className="flex flex-wrap gap-2" {...fadeUp(0.2)}>
            {TAGS.map((tag) => (
              <span
                key={tag}
                className="text-xs font-mono font-bold uppercase tracking-widest px-3 py-1.5 rounded border border-border-dark text-slate-400 bg-white/[0.02] transition-colors duration-200 hover:border-accent-blue/40 hover:text-slate-200"
              >
                {tag}
              </span>
            ))}
          ...
          {/* Divider line — crimson → cyan */}
          <motion.div
            className="w-24 h-px bg-gradient-to-r from-primary to-accent-blue"
          ...
          <motion.p
            ...
            {...fadeUp(0.35)}
          >
            A broad-range engineer moving fluently across AI, data science, and
            full-stack software — building intelligent systems and scalable solutions
            with the discipline of a high-performance athlete.
          </motion.p>

          {/* CTAs */}
          <motion.div className="flex flex-wrap items-center gap-3 pt-1" {...fadeUp(0.5)}>
            <MagneticButton>
              <Link
                href="#projects"
                className="group flex items-center gap-2.5 h-12 px-6 bg-primary text-white text-sm font-bold rounded-lg transition-all duration-300 hover:bg-primary-hover hover:shadow-[0_0_24px_rgba(225,29,58,0.4)] active:scale-95"
              >
                View Projects
            ...
            <a
              ...
              className="flex items-center gap-2.5 h-12 px-6 border border-accent-blue/30 text-slate-200 text-sm font-bold rounded-lg transition-all duration-300 hover:border-accent-blue/60 hover:bg-accent-blue/[0.06] hover:scale-[1.02] active:scale-95"
            >
              <FaFileAlt size={13} />
              Resume
            </a>
